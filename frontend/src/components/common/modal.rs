// frontend/src/components/common/modal.rs
use leptos::*;
...
pub fn Modal(
    ...
    #[prop(optional)] size: Option<String>, // "sm", "md", "lg", "xl"
    children: Children,
) -> impl IntoView {
    ...
    view! {
        <Show when=move || show.get()>
            <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
                <div class={format!("relative bg-white rounded-2xl shadow-2xl w-full {} max-h-[90vh] overflow-y-auto border border-gray-200", modal_classes)}>
                    <Show when=move || title.is_some()>
                        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 rounded-t-2xl bg-gray-50">
